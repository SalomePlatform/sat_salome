...
echo ##########################################################################
echo openturns 1.25.1
echo ##########################################################################
SET CMAKE_MODULE_OPTIONS=%CMAKE_OPTIONS%
CALL:MODULE_BUILDER openturns "%SOURCE_DIR%\openturns-1.25.1" "%BUILD_DIR%\openturns" "%PRODUCT_INSTALL%" "%CMAKE_MODULE_OPTIONS%"

set PATH=%PRODUCT_INSTALL%\bin;%PRODUCT_INSTALL%\lib;%PATH%
set PYTHONPATH=%PRODUCT_INSTALL%\lib\site-packages;%PYTHONPATH%

echo ##########################################################################
echo otagrum 0.12 [SKIPPED]
echo ##########################################################################
REM SET CMAKE_MODULE_OPTIONS=%CMAKE_OPTIONS% -DBUILD_DOC=OFF
REM CALL:MODULE_BUILDER otagrum "%SOURCE_DIR%\otagrum-0.12" "%BUILD_DIR%\otagrum" "%PRODUCT_INSTALL%" "%CMAKE_MODULE_OPTIONS%"

echo ##########################################################################
echo otmorris 0.18
echo ##########################################################################
SET CMAKE_MODULE_OPTIONS=%CMAKE_OPTIONS%
CALL:MODULE_BUILDER otmorris "%SOURCE_DIR%\otmorris-0.18" "%BUILD_DIR%\otmorris" "%PRODUCT_INSTALL%" "%CMAKE_MODULE_OPTIONS%"

echo ##########################################################################
echo otfftw 0.17
echo ##########################################################################
SET CMAKE_MODULE_OPTIONS=%CMAKE_OPTIONS%
CALL:MODULE_BUILDER otfftw "%SOURCE_DIR%\otfftw-0.17" "%BUILD_DIR%\otfftw" "%PRODUCT_INSTALL%" "%CMAKE_MODULE_OPTIONS%"

echo ##########################################################################
echo otmixmod 0.19 [SKIPPED]
echo ##########################################################################
SET CMAKE_MODULE_OPTIONS=%CMAKE_OPTIONS%
REM CALL:MODULE_BUILDER otmixmod "%SOURCE_DIR%\otmixmod-0.19" "%BUILD_DIR%\otmixmod" "%PRODUCT_INSTALL%" "%CMAKE_MODULE_OPTIONS%"

echo ##########################################################################
echo otrobopt 0.16
echo ##########################################################################
SET CMAKE_MODULE_OPTIONS=%CMAKE_OPTIONS% -DBUILD_DOC=OFF
CALL:MODULE_BUILDER otrobopt "%SOURCE_DIR%\otrobopt-0.16" "%BUILD_DIR%\otrobopt" "%PRODUCT_INSTALL%" "%CMAKE_MODULE_OPTIONS%"

echo ##########################################################################
echo otsvm 0.16
echo ##########################################################################
SET CMAKE_MODULE_OPTIONS=%CMAKE_OPTIONS% -DBUILD_DOC=OFF
CALL:MODULE_BUILDER otsvm "%SOURCE_DIR%\otsvm-0.16" "%BUILD_DIR%\otsvm" "%PRODUCT_INSTALL%" "%CMAKE_MODULE_OPTIONS%"
...
set CMAKE_OPTIONS_EXT=%CMAKE_OPTIONS%
cd  %BUILD_DIR%\otpod
xcopy %SOURCE_DIR%\otpod-0.6.12\*   %BUILD_DIR%\otpod /E /I /Q
if NOT %ERRORLEVEL% == 0 (
    echo ERROR on xcopy
    exit 3
)
