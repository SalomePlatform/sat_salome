...
CONFIGURE_FLAGS=
CONFIGURE_FLAGS+="--download-slepc=ext/slepc-3.24.1.tar.gz"
...
if [ "$SAT_hdf5_IS_NATIVE" == "1" ]; then
    ...
    if [[ "$LINUX_DISTRIBUTION" == "FD4"* ]] || [[ "$LINUX_DISTRIBUTION" == "CO10" ]]; then
...
if [ -f "${NATIVE_PATH}/libmetis.so" ] && [ "${SAT_metis_IS_NATIVE}" == "1" ]; then
    ...
    CONFIGURE_FLAGS+=" --download-metis=ext/metis-5.1.0-p12.tar.gz"
fi
...
if [ -n "${SAT_HPC}" ]; then
  ...
  if [ -f "${NATIVE_PATH}/libHYPRE_core.so" ]; then
      ...
      CONFIGURE_FLAGS+=" --download-hypre=ext/hypre-3.0.0.tar.gz"
  fi
  ...
  if [ -f "${NATIVE_PATH}/libptscotch.so" ] && [ "${SAT_ptscotch_IS_NATIVE}" == "1" ]; then
      ...
      CONFIGURE_FLAGS+=" --download-ptscotch=ext/scotch-v7.0.10.tar.gz"
  fi

  if [ -n "${MPI4PY_ROOT_DIR}" ]; then
      CONFIGURE_FLAGS+=" --with-mpi4py-dir=${MPI4PY_ROOT_DIR}"
  else
      CONFIGURE_FLAGS+=" --download-mpi4py=ext/mpi4py-4.1.0.tar.gz"
  fi
